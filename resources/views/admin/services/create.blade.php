...
@section('content')
<h1 class="font-display font-bold text-2xl text-navy mb-6">Nouveau service</h1>
<form method="POST" action="{{ route('admin.services.store') }}" class="space-y-6 bg-white p-6 rounded-lg shadow-sm border border-gray-200">
  @csrf
  ...
  <div class="grid md:grid-cols-2 gap-4">
    <div>
      ...
      <textarea name="summary_fr" rows="3" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm">{{ old('summary_fr') }}</textarea>
    </div>
    <div>
      <label class="block text-sm font-semibold text-gray-700 mb-1">Résumé (EN) *</label>
      <textarea name="summary_en" rows="3" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm">{{ old('summary_en') }}</textarea>
    </div>
  </div>
  <div class="grid md:grid-cols-2 gap-4">
    <div>
      <label class="block text-sm font-semibold text-gray-700 mb-1">Corps (FR) *</label>
      <textarea name="body_fr" rows="8" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm font-mono">{{ old('body_fr') }}</textarea>
    </div>
    <div>
      <label class="block text-sm font-semibold text-gray-700 mb-1">Corps (EN) *</label>
      <textarea name="body_en" rows="8" required class="w-full border border-gray-300 rounded px-3 py-2 text-sm font-mono">{{ old('body_en') }}</textarea>
    </div>
  </div>
  <div class="flex gap-4">
    <div>
      <label class="block text-sm font-semibold text-gray-700 mb-1">Icône (emoji)</label>
      <input name="icon" value="{{ old('icon', '🏢') }}" class="border border-gray-300 rounded px-3 py-2 text-sm w-20" />
    </div>
    <div>
      <label class="block text-sm font-semibold text-gray-700 mb-1">Ordre</label>
      <input name="order" type="number" value="{{ old('order', 0) }}" class="border border-gray-300 rounded px-3 py-2 text-sm w-20" />
    </div>
  </div>
